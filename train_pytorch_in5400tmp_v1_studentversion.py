...
def traineval2_model_nocv(dataloader_train, dataloader_test ,  model ,  criterion, optimizer, scheduler, num_epochs, device, numcl):
  ...
  for epoch in range(num_epochs):
    ...
    avgtestperfs.append(avgperfmeasure)
...
class wsconv2(nn.Conv2d):
  ...
  def forward(self,x):
    ...
    W = self.weight
    s = [np.std(W[c,:,:,:]) for c in range(self.out_channels)]
    n = [np.sqrt(s[c]**2+self.eps) for c in range(self.out_channels)]
    W_hat = [(W[c,:,:,:]/(n[c])) for c in range(self.out_channels)]

    return F.conv2d(x, W_hat, bias=self.bias, stride=self.stride, padding=self.padding, 
