...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_state_size, last_layer_state_size, num_rnn_layers, cell_type='LSTM'):
        ...
        if self.cell_type == 'GRU':
            ...
            for i in range(self.num_rnn_layers):
                if i == self.num_rnn_layers - 1:
                    self.cells.append(GRUCell(hidden_state_size=input_size_list[i+1], input_size=self.last_layer_state_size))
                else:
            ...
            for i in range(self.num_rnn_layers):
                if i == self.num_rnn_layers - 1:
                    self.cells.append(LSTMCell(hidden_state_size=input_size_list[i+1], input_size=self.last_layer_state_size))
                else:
            ...
            for i in range(self.num_rnn_layers):
                if i == self.num_rnn_layers - 1:
                    self.cells.append(RNNsimpleCell(hidden_state_size=input_size_list[i+1], input_size=self.last_layer_state_size))
                else:
    ...
    def forward(self, xTokens, baseimgfeat, initial_hidden_state, attentionlayer, outputlayer, Embedding, is_train=True):
        ...
        for kk in range(seqLen):
            ...
            m = nn.MaxPool1d(10)
            max_baseimgfeat = m(baseimgfeat).squeeze()
            print(max_baseimgfeat.shape)
            exit()
